...
La statistique de test est de 11.38, ce qui est largement supérieure aux valeurs critiques.
...
```{r}
adf_sp_aug <- ur.df(pib_sp, type = "drift", selectlags = "AIC")
summary(adf_sp_aug)
```

La statistique de test, 0.526, est beaucoup plus grande que les valeurs critiques.
Ici aussi nous ne pouvons pas rejetter l’hypothèse de racine unitaire.

La série semble toujours non stationnaire, même en tenant compte d’une constante.
De plus, le coefficient z.lag.1 est non significatif, cela renforce l'idée que la série a une racine unitaire.
...
### Test sans tendance ni constante
...
Alors, nous ne pouvons pas rejetter l’hypothèse de racine unitaire.
Ici, le t-stat étant très positif, la série est fortement non stationnaire, avec une présence de racine unitaire.
Nous allons alors ajouter une constante.
...
Avec l'ajout d'une constante, nous voyons que les valeurs du t-stat sont encore supérieures aux valeurs critiques.
Ainsi, nous effectuerons le test en ajoutant une tendance.
...
Dans ce cas, nous observons que les valeurs t-stat sont bien supérieures aux valeurs critiques pour tau3 et phi2, cependant, ce n'est pas le cas pour phi3.
...
```{r}
adf_pg_aug <- ur.df(pib_pg, type = "drift", selectlags = "AIC")
summary(adf_pg_aug)
```
